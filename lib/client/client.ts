...
export class MetaDAORPClient implements MetaDAOClient {
    ...
    constructor(programVersion: ProgramVersion, rpcProvider: Provider) {
    ...
    async fetchDao(daoAddress: string): Promise<Dao | undefined> {
        ...
        const autocratProgram =  new Program<AutocratProgram>(idl as AutocratProgram, programId, this.rpcProvider),
        const daoState = await autocratProgram.account.dao.fetch(dao);
        const baseToken = await this.metaplex.nfts().findByMint({mintAddress: daoState.tokenMint})

        if (daoState && baseToken.tokenStandard === TokenStandard.Fungible && baseToken.json?.symbol) {
            return {
                daoState,
                baseToken: {
