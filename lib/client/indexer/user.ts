...
export class FutarchyIndexerUserClient implements FutarchyUserClient {
  ...
  async fetchUserPerformance(request: UserPerformanceFetchRequest): Promise<UserPerformance[]> {
    const { user_performance } = await this.graphqlClient.query?.({
      user_performance: {
        __args: {
          where: {
            user_acct: { _eq: request.userAcct.toString() }
          },
        },
        user_acct: true,
        proposal_acct: true,
    ...
    return user_performance.map<UserPerformance | null>((p) => {
      ...
      const volume = PriceMath.getHumanAmount(
      ...
      const pnl = PriceMath.getHumanAmount(
        new BN(p.volume_bought),
        new BN(proposal.dao.tokenByQuoteAcct?.decimals ?? 6)
      ) - PriceMath.getHumanAmount(
        new BN(p.volume_sold),
