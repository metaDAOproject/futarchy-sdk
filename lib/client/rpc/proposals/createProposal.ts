...
import { Dao, DaoAggregate, ProgramVersionLabel, ProposalInstructionWithPreinstructions } from "@/types";
...
import { AutocratClient, MaxCUs, getAmmAddr, getVaultAddr, getVaultFinalizeMintAddr, getVaultRevertMintAddr } from "@metadaoproject/futarchy-ts";
...
import { OpenbookTwapV0_2, IDL as OPENBOOK_TWAP_IDLV0_2 } from "@/idl/openbook_twap_v0.2";
...
export class CreateProposalClient implements CreateProposal {
    ...
    private async createProposalV0_2(dao: Dao, onPassIx: ProposalInstructionWithPreinstructions, marketParams: OpenbookMarketParams): SendTransactionResponse {
        ...
        const vaultProgram = new Program<Conditional_vault_v0_2>(vault.idl as Conditional_vault_v0_2, vault.programId, this.rpcProvider)

        const openbook = new OpenBookV2Client(this.rpcProvider);
        const twap = autocratVersionToTwapMap["V0.2"]
        const twapProgram = new Program<OpenbookTwapV0_2>(twap.idl as OpenbookTwapV0_2, twap.programId, this.rpcProvider);
    ...
    public async createProposal(
        ...
        switch (version) {
            case "V0.2":
                return resp = await this.createProposalV0_2(currentDao, onPassIxs, marketParams as OpenbookMarketParams)
            case "V1":
                return resp = await this.createProposalV1(currentDao, onPassIxs, marketParams as AmmMarketParams)
            default:
