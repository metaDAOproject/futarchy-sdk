...
import { AUTOCRAT_VERSIONS, autocratVersionToConditionalVaultMap } from "@/constants";
...
import { AutocratClient, getAmmAddr, getAmmLpMintAddr, getDaoTreasuryAddr, getVaultAddr, getVaultFinalizeMintAddr, getVaultRevertMintAddr } from "@metadaoproject/futarchy-ts";
...
export class FinalizeProposalClient implements FinalizeProposal {
    ...
    constructor(proposalsClient: FutarchyProposalsClient, rpcProvider: AnchorProvider, autocratClient: AutocratClient, transactionSender: TransactionSender | undefined) {
    ...
    private async finalizeProposalv02( proposal: Proposal) {
        const autocratProgramId = AUTOCRAT_VERSIONS.find((a) => a.label == "V0.2")?.programId
        if(!autocratProgramId) return
        const autocrat = new Program<AutocratV0_2>(AutocratV0_2_IDL, autocratProgramId, this.rpcProvider)
        
        // @ts-ignore
        const proposalAccount = await autocrat.account.proposal.fetch(proposal.publicKey)

        console.log(proposalAccount, proposalAccount.dao)
        //@ts-ignore
        ...
        // @ts-ignore
        const storedProposal = await autocrat.account.proposal.fetch(proposal.publicKey);
        ...
        return await this.transactionSender?.send([tx], this.rpcProvider.connection)
    }
    ...
    private async finalizeProposalIx(
        ...
        const [daoTreasury] = getDaoTreasuryAddr(this.autocratClient.autocrat.programId, dao);
    ...
    private async finalizeProposalv1(proposal: Proposal){
        try{
            const proposalAccount = await this.autocratClient.getProposal(proposal.publicKey)
            const dao = await this.autocratClient.getDao(proposalAccount.dao)
            const finalizeProposalTx = await (await this.finalizeProposalIx(proposal.publicKey, proposalAccount.instruction, proposalAccount.dao, dao.tokenMint, dao.usdcMint, proposalAccount.proposer)).transaction()

            return await this.transactionSender?.send([finalizeProposalTx], this.rpcProvider.connection)
        }catch(e){
            console.log("error", e)
        }
    }
