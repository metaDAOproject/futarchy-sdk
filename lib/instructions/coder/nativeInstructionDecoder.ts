...
export class NativeInstructionDecoder implements DecodeInstructionCoder {
  private ixLayout: Map<string, Layout<Object>>;
  private sighashLayouts: Map<string, { layout: Layout<Object>, name: string }>;
    ...
    if (idl.instructions) {
      idl.instructions.forEach((ix, idx) => {
        ...
        sighashLayouts.set(bs58.encode([idx]), {
          layout: this.ixLayout.get(ixName) as Layout<Object>,
          name: ix.name,
        });
      });
    }
