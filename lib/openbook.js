...
const findOpenOrdersIndex = async ({ signer, indexOffset, openbook, }) => {
    const openTx = new web3_js_1.Transaction();
    const openOrdersIndexer = (0, exports.findOpenOrdersIndexer)(signer);
    let accountIndex = new anchor_1.BN(1);
    try {
        const indexer = await openbook.account.openOrdersIndexer.fetch(openOrdersIndexer);
        accountIndex = new anchor_1.BN(((indexer === null || indexer === void 0 ? void 0 : indexer.createdCounter) || 0) + 1 + (indexOffset || 0));
    }
    catch {
        if (!indexOffset) {
            openTx.add(await (0, exports.createOpenOrdersIndexerInstruction)(openbook, openOrdersIndexer, signer));
        }
        else {
            accountIndex = new anchor_1.BN(1 + (indexOffset || 0));
        }
    }
    return [accountIndex, openTx];
};
exports.findOpenOrdersIndex = findOpenOrdersIndex;
